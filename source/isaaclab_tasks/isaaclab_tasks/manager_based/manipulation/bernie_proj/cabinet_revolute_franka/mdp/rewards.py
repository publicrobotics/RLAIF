...
# Revolute joint Rewards:
def open_revolute_bonus(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Bonus for opening a revolute joint using default (closed) position."""

    asset = env.scene[asset_cfg.name]

    joint_id = asset_cfg.joint_ids[0]

    joint_pos = asset.data.joint_pos[:, joint_id]
    default_pos = asset.data.default_joint_pos[:, joint_id]

    is_graspable = align_grasp_around_handle(env).float()

    opening_amount = torch.abs(joint_pos - default_pos)

    return (is_graspable + 1.0) * opening_amount


def multi_stage_open_revolute(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg) -> torch.Tensor:
    """Multi-stage reward using deviation from default joint position."""
    ...
    open_easy = (opening_amount > 0.05) * 0.5
    open_medium = (opening_amount > 0.3) * is_graspable
    open_hard = (opening_amount > 0.5) * is_graspable
